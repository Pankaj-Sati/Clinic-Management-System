...
<br>
<?php
//------------First we will check whether the no_of_sessions in checkup record is not 0--------//

if($checkup_record_obj->no_of_sessions==0)
{
	//This means that the checkup record has been completed. Still the user is trying to add new data. So we will warn him
	printf("%s","<blockquote><p>".SESSIONS_EXPIRED_ERROR."</p></blockquote><br><br>");
	//Php can still work further
}

?>

<!-- ---------If everything works well till now,we will make form to add new session data------------------- -->
<h4>Sessions Left=<?php printf("%d",$checkup_record_obj->no_of_sessions);?></h4>
Fields marked with * are compulsary <br><br>
	<form action="insert_new_checkup_session.php" method="post">
		<p><label for="name">Session Date*: </label> <input type="date" value="<?php printf('%s',date('Y-m-d'))?>" required name="session_date"/></p>
		<p><label for="name">Remarks: </label><input type="text" name="session_remarks"/></p>
		<input type="hidden" name="cr_id" value="<?php printf("%d",$passed_cr_id);?>" />
		<a href="get_checkup_sessions.php?cr_id=<?php printf("%d",$passed_cr_id);?>" class="button">Back To Checkup</a>
